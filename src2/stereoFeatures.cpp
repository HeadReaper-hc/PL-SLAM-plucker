...
LineFeature::LineFeature( Vector2d spl_, double sdisp_, Vector3d sP_, Vector2d spl_obs_, double sdisp_obs_,
                          ...
                          Matrix3d covE_an_, Matrix3d covS_an_, Vector6d nd_) :
    ...
    le(le_), le_obs(le_obs_), angle(angle_), idx(idx_), level(level_), inlier(inlier_), sigma2(sigma2_), covE_an(covE_an_), covS_an(covS_an_), NDc(nd_)
{
    for( int i = 0; i < level; i++ )
        sigma2 *= Config::lsdScale();
    sigma2 = 1.f / (sigma2*sigma2);
...
LineFeature* LineFeature::safeCopy(){
    return new LineFeature( spl, sdisp, sP, spl_obs, sdisp_obs,
                            ...
                            le, le_obs, angle, idx, level, inlier, sigma2, covE_an, covS_an, NDc );
}
...
LineFeature::LineFeature( Vector2d spl_, double sdisp_, Vector3d sP_,
        Vector2d epl_, double edisp_, Vector3d eP_,
        Vector3d le_,  double angle_, int idx_, int level_, Vector6d nd_) :
        spl(spl_), sdisp(sdisp_), sP(sP_), epl(epl_), edisp(edisp_), eP(eP_), le(le_), inlier(true), idx(idx_), angle(angle_), level(level_), NDc(nd_)
{
    for( int i = 0; i < level; i++ )
        sigma2 *= Config::lsdScale();
        sigma2 = 1.f / (sigma2*sigma2);
